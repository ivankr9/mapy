...
class myButton(QPushButton):
	def __init__(self, parent=None,d=False,c=False,t=False,*args, **kwargs):
		...
		if c:
			...
			self.setStyleSheet("QPushButton{background-color:"+c1+"; color: black; border: black 2px} QPushButton:hover{background-color:"+c2+"; color: black; border: black 2px} QPushButton:pressed{background-color:"+c3+"; color: black; border: black 2px}")
		if t:
...
class mapyLayout(QWidget):
	...
	def dynLayout(self):
		...
		gridLayout.addWidget(button4, 1,1)
		button5 = myButton("mkCache",lambda: makeNCache(), (0.6, 0.4, 0.2), 'Make nCache\n(Selected)\nSetup All Nucleus Nodes to Start Frame with preroll from _DYN_ Extra Attrib')
		gridLayout.addWidget(button5, 2,1)
...
def Setup_Dyn():
	...
	if len(cmds.ls("|_DYN_",r=True)):
		...
		mel.eval('addAttr -ln "anim_frame_start"  -at long  -dv '+str(int(fs))+' |_DYN_;')
		mel.eval('setAttr -e-keyable true |_DYN_.anim_frame_start;')
		mel.eval('addAttr -ln "anim_frame_end"  -at long  -dv '+str(int(fe))+' |_DYN_;')
...
def makeDynABC():
	...
	ffolder = os.path.dirname(fname)
	scname = os.path.basename(fname).split('.')[0]
	abcdynname = ffolder+'/cache/dyn/'+scname+'_dyn.abc'
	...
	for s in selected:
		sel += ' -root ' + s
	melcommand = 'AbcExport -j "-frameRange '+str(FS)+' '+str(FE)+' -uvWrite -worldSpace -eulerFilter -dataFormat ogawa '+sel+' -file '+abcfile_path+'";'
	mel.eval('refresh -suspend 1;')
	try:
		mel.eval(melcommand)
	except:
